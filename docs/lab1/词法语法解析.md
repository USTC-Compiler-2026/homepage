...
为了大家方便，我们提供了 Cminusf 语法的 pdf 文件，可在此下载：[Cminusf 语法](./assets/cminusf.pdf)
...
```shell
obug@obug-VMware-Virtual-Platform:~/2026ustc-jianmu-compiler/build$ make
...
进入 `2026ustc-jianmu-compiler` 仓库下的 `src/parser` 目录，运行以下命令
