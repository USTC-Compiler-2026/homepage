...
!!! warning "节点序号"
    ...
    在介绍算法的论文中，支配节点的序号大于被支配节点，对应到我们的实验中应为 `post_order`，上图所示的小于号成立；如果你查找序号使用的是 `reversed_post_order`，应该相应地改为大于号。这个函数的差异不影响其他 TODO，自行对应即可。

<hr class="hr-my" data-content="(●′∀｀●) 我是分隔线 (●′∀｀●)">


然后按照逆后序进行数据流迭代，计算直接支配节点，

![alt text](image-4.png)

在这个算法的实现中，每次迭代，将节点的直接支配节点设置为 `idom` 非空的前驱的公共祖先。**非空** 主要是为了解决以下情况：
