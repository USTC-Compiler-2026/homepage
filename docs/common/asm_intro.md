...
以[示例 2](#示例-2)（非栈式分配）中的汇编文件为例，下图展示了一个汇编文件的整体结构与组成：
...
在龙芯架构中，所有指令均采用 32 位固定长度，且指令的地址都要求 4 字节边界对齐（即指令的地址可以被 4 字节整除）。如下，为某个汇编程序进入函数体时，首条汇编指令及后 2 条指令的地址，可见指令的地址按照 4 字节边界对齐：
...
- 注意事项：
  ...
  - 上述指令搭配 `ori` 指令（在后文介绍），可以将超过 12 位的立即数装入通用寄存器中
...
!!! info "跳转目标的表示"
    ...
    在汇编代码中，分支指令的跳转目标地址可以通过相应标签或使用字节为单位的偏移量表示。

	由于龙芯一条指令 4 个字节，机械码中的立即数偏移 offs 以 4 字节为单位（offs = 0x1 代表偏移 0x4 字节），这样比以字节为单位表示的范围更大。但如果在汇编中使用偏移量表示目标地址，那汇编中的立即数字段仍然填入以字节为单位的偏移值，对应 `offs << 2`。
...
- 指令格式：
  ...
  bgeu $rj, $rd, offs16
  ```
注意汇编中填入 `offs16 << 2` 或 `label` 而非 `offs16`。
...
- 指令格式：
  ...
  bnez $rj, offs21
  ```
注意汇编中填入 `offs21 << 2` 或 `label` 而非 `offs21`。
...
注意汇编中填入 `offs26 << 2` 或 `label` 而非 `offs26`。
...
???+ info "栈帧对齐"
	...
	例如某函数栈帧为空，则栈帧只包含调用者的 `$ra` 和 `$fp` 备份，该函数应该满足 `$sp = $fp - 16`。但如果栈帧中放置了一个 `i32`，考虑到对齐需求，栈帧将会变为

	![alt text](./figs/padstack.png)
...
下面的汇编代码可能与实验需要实现的编译器的生成结果有所差异，**它们均使用寄存器而非栈式分配**。
